<x-app-layout>
    ...
    {{-- Judul Halaman --}}
    <x-slot name="title">Edit Data Barang</x-slot>

    {{-- Bagian Edit Data Barang --}}
    <section class="max-w-xl mx-auto bg-white p-6 rounded-xl shadow">
        <h2 class="text-xl font-semibold text-gray-800 mb-6">Form Edit Data Barang</h2>

        <form method="POST" action="{{ route('dashboard.item.update', $item->id) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="description" name="description" required rows="2"
                    class="mt-1 w-full px-2 py-1.5 text-sm border border-gray-300 rounded-lg shadow-sm focus:ring focus:ring-indigo-200 focus:border-indigo-500 transition">{{ old('description', $item->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <div
                        ...
                        <input id="sku" name="sku" type="number"
                            value="{{ old('sku', ltrim(str_replace('SKU-', '', $item->sku), '0')) }}"
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input id="stock" name="stock" type="number" value="{{ old('stock', $item->stock) }}"
                            required min="0"
